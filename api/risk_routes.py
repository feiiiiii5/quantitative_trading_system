...
position_mgr = DynamicPositionManager()
stop_loss_mgr = MultiDimensionStopLoss()
stress_test = RiskStressTest()
...
async def calculate_portfolio_var(
    ...
    try:
        ...
        result = var_monitor.calculate_portfolio_var(positions, ret_arrays)
        return _resp(True, data=result.to_dict())
    except Exception as e:
        return _resp(False, msg=str(e))
...
async def calculate_greeks(
    symbol: str = Query(...),
    S: float = Query(..., description="标的现价"),
    K: float = Query(..., description="行权价"),
    T: float = Query(..., description="到期时间(年)"),
    r: float = Query(0.03, description="无风险利率"),
    sigma: float = Query(0.3, description="隐含波动率"),
    option_type: str = Query("call"),
):
    result = var_monitor.calculate_option_greeks(symbol, S, K, T, r, sigma, option_type)
    return _resp(True, data=result.to_dict())
...
async def calculate_position(
    ...
    try:
        ...
        result = position_mgr.calculate_position(
            symbol, capital, price, atr, volatility, win_rate,
        )
        return _resp(True, data=result.to_dict())
    except ValueError:
...
async def get_position_mode_info():
    return _resp(True, data=position_mgr.get_mode_info())
...
async def set_position_constraints(
    ...
    position_mgr.constraints = PositionConstraint(
...
async def get_portfolio_risk(positions: str = Query(..., description="JSON格式的持仓数据")):
    try:
        import json
        pos_data = json.loads(positions)
        result = position_mgr.get_portfolio_risk(pos_data)
...
async def set_stop_loss(
    ...
    try:
        ...
        order = stop_loss_mgr.set_stop_loss(symbol, entry_price, entry_date, st, params)
...
async def set_take_profit(
    ...
    order = stop_loss_mgr.set_take_profit(symbol, entry_price, take_profit_pct)
    return _resp(True, data=order.to_dict())
...
async def check_stop_loss(
    ...
    sl_result = stop_loss_mgr.check_stop_loss(symbol, current_price, current_date)
    tp_result = stop_loss_mgr.check_take_profit(symbol, current_price)
...
async def check_circuit_breaker(
    ...
    result = stop_loss_mgr.check_circuit_breaker(current_equity, trade_pnl)
    return _resp(True, data=result)


@router.get("/stoploss/orders")
async def get_active_orders():
    orders = stop_loss_mgr.get_active_orders()
...
async def get_stress_scenarios():
    scenarios = stress_test.get_scenarios()
...
async def run_stress_test(
    ...
    try:
        import json
        pos_data = json.loads(positions)
        result = stress_test.run_scenario(scenario_name, pos_data)
        return _resp(True, data=result.to_dict())
    except Exception as e:
        return _resp(False, msg=str(e))
...
async def run_all_stress_tests(positions: str = Query(..., description="JSON格式的持仓数据")):
    try:
        import json
        pos_data = json.loads(positions)
        results = stress_test.run_all_scenarios(pos_data)
...
async def run_custom_stress(
    ...
    try:
        import json
        pos_data = json.loads(positions)
        result = stress_test.run_custom_shock(pos_data, shock_pct, volatility_mult, liquidity_shock)
        return _resp(True, data=result.to_dict())
    except Exception as e:
        return _resp(False, msg=str(e))
...
async def brinson_attribution(
    ...
    try:
        ...
        result = risk_attr.brinson_attribution(pr, br, pw, bw)
        return _resp(True, data=result.to_dict())
    except Exception as e:
        return _resp(False, msg=str(e))
...
async def barra_exposures(returns: str = Query(..., description="逗号分隔的收益率序列")):
    try:
        ret_array = np.array([float(r) for r in returns.split(",") if r.strip()])
        exposures = risk_attr.calculate_barra_exposures(ret_array)
...
async def generate_risk_report(returns: str = Query(..., description="逗号分隔的收益率序列")):
    try:
        ret_array = np.array([float(r) for r in returns.split(",") if r.strip()])
        report = risk_attr.generate_risk_report(ret_array)
