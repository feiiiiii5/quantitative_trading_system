...
decoupler = SignalExecutionDecoupler()
ml_module = MLStrategyModule()
factor_wb = FactorResearchWorkbench()
version_ctrl = StrategyVersionControl()
...
async def create_strategy(name: str = Query(...), description: str = Query("")):
    strategy = builder.create_strategy(name, description)
...
async def add_node(
    ...
    try:
        ...
        p = {}
    node = builder.add_node(strategy_name, node_type, node_name, p)
    if node:
...
async def add_edge(
    ...
    edge = builder.add_edge(strategy_name, source_id, source_output, target_id, target_input)
    if edge:
...
async def export_strategy(name: str):
    code = builder.export_to_python(name)
    if code:
...
async def get_ml_models():
    models = ml_module.get_available_models()
    return _resp(True, data=models)


@router.get("/ml/features")
async def get_ml_features():
    features = ml_module.get_features_info()
    return _resp(True, data=features)


@router.post("/ml/train")
async def train_ml_model(symbol: str = Query(...), period: str = Query("1y"), model_type: str = Query("lightgbm")):
    from core.data_fetcher import SmartDataFetcher
    fetcher = SmartDataFetcher()
    try:
        ...
        result = ml_module.train_model(df, model_type)
...
async def research_factor(
    ...
    try:
        ...
        result = factor_wb.research_factor(name, fv, ret, forward_period)
...
async def composite_factor(
    ...
    try:
        ...
        result = factor_wb.multi_factor_composite(factors, ret, method)
        return _resp(True, data=result)
    except Exception as e:
        return _resp(False, msg=str(e))
...
async def commit_version(
    ...
    try:
        ...
        p = {}
    version = version_ctrl.commit(strategy_name, p, commit_message, author)
    return _resp(True, data=version.to_dict())


@router.get("/version/list/{strategy_name}")
async def list_versions(strategy_name: str):
    versions = version_ctrl.get_versions(strategy_name)
    return _resp(True, data=versions)


@router.get("/version/latest/{strategy_name}")
async def get_latest_version(strategy_name: str):
    version = version_ctrl.get_latest(strategy_name)
    if version:
...
async def diff_versions(strategy_name: str, v1: str = Query(...), v2: str = Query(...)):
    result = version_ctrl.diff_versions(strategy_name, v1, v2)
    return _resp(True, data=result)


@router.post("/version/promote/{strategy_name}")
async def promote_version(strategy_name: str, version_id: str = Query(...)):
    result = version_ctrl.promote(strategy_name, version_id)
    return _resp(result.get("success", False), data=result)


@router.get("/version/strategies")
async def list_all_strategies():
    strategies = version_ctrl.list_strategies()
    return _resp(True, data=strategies)
